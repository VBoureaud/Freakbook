...
void isAdult_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void isAdult_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "isAdult";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 2;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
PFrElement aux_dest = &lvar[0];
// load src
Fr_div(&expaux[1],&signalValues[mySignalStart + 2],&circuitConstants[5]); // line circom 16
Fr_div(&expaux[2],&signalValues[mySignalStart + 1],&circuitConstants[5]); // line circom 16
Fr_sub(&expaux[0],&expaux[1],&expaux[2]); // line circom 16
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
isAdult_2_create(1,0,ctx,"main",0);
isAdult_2_run(0,ctx);
}
